...
float _Drag;
float _ShiverDrag;
float _ShiverPositionBusality;
float _WindNormalInfluence;
float4 _NewNormal;

float4  WIND_SETTINGS_WorldPositionBusAndSpeed;
...
{
    WindData result;
    float3 normalizedDir = normalize(WIND_SETTINGS_WorldPositionBusAndSpeed.xyz);

    float3 worldOffset = normalizedDir * WIND_SETTINGS_WorldPositionBusAndSpeed.w * time.y;
    ...
    if(WIND_SETTINGS_WorldPositionBusAndSpeed.w > 0.0 || WIND_SETTINGS_Turbulence > 0.0)
    {
    ...
    float3 trunkNoise =
        (
                (normalizedDir * WIND_SETTINGS_WorldPositionBusAndSpeed.w)
    ...
    result.PositionBus = dir;
    result.ShiverPositionBus = shiverNoise;
                            ...
                            float4          time)
{
    ...
    if (wind.Strength > 0.0)
    {
        ...
        float3 rotAxis = cross(float3(0, 1, 0), wind.PositionBus);

        positionWS = Rotate(rootWP, positionWS, rotAxis, (wind.Strength) * 0.001 * att);

        float3 shiverPositionBus = normalize(lerp(normalWS, normalize(wind.PositionBus + wind.ShiverPositionBus), shiverPositionBusality));
        positionWS += wind.ShiverStrength * shiverPositionBus * shiverMask;
    }
	windData = wind;
...
void vert (inout appdata_full v) {
	...
	ApplyWindDisplacement( positionWS, windData, normalWS, rootWP, _Stiffness, _Drag, _ShiverDrag, _ShiverPositionBusality, _InitialBend, v.color.a, _Time);
	...
	if (_WindNormalInfluence != 0)
		v.normal.y +=lerp(-_WindNormalInfluence, _WindNormalInfluence, windData.ShiverStrength);
